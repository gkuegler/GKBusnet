...
from modbus_utils import crc16, set_bit
import re
import socket
import select
import struct
...
class ModbusClient:
    ...
    def __init__(self, host=None, port=None, unit_id=None, timeout=None,
        ...
        if not host: raise ValueError('No Host Given for Device')
        # set port
        if port:
            if not self.port(port):
                raise ValueError('port value error')
        if not port: raise ValueError('No Port Given for Device')
        # set unit_id
    ...
    def _send_mbus(self, frame):
        ...
        :param frame: modbus frame to send (with MBAP for TCP)
