...
class ModbusServer(object):
    ...
    class ModbusService(BaseRequestHandler):
        ...
        def handle(self):
            while True:
                ...
                elif rx_bd_fc is const.ECHO: tx_body = struct.unpack('>HH', rx_body[1:])
    ...
    def __init__(self, host=None, port=None, no_block=False, ipv6=False):
        """Constructor
...
if __name__ == "__main__":
    ...
    def cmdlist():
        print('\nr      restart server')
        print('s      kill blocking server')
        x = input('Command: ')

        if x=='r':
            try: testserver.stop()
            except: starttestserver()
        if x=='s':
            try: testserver.stop()
            except:
                print('\nServer Threw Exception. This happens if already stopped.')
                cmdlist()



    def starttestserver():
        testserver = ModbusServer(host='127.0.0.1', port=62500)
        print('\nHost: {}'.format(testserver.host))
        print('Port: {}'.format(testserver.port))
